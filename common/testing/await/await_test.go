...
func TestRequire_ImmediateSuccess(t *testing.T) {
	called := 0
	Require(t.Context(), t, func(_ context.Context, t *T) {
		called++
...
func TestRequire_PropagatesParentContextValues(t *testing.T) {
	...
	Require(ctx, t, func(ctx context.Context, t *T) {
...
func TestRequire_ScalesTimeoutContextDeadline(t *testing.T) {
	...
	Require(t.Context(), t, func(ctx context.Context, t *T) {
...
func TestRequire_TimeoutFailsTest(t *testing.T) {
	tb := runWithFakeTB(func(tb *fakeTB) {
		Require(context.Background(), tb, func(_ context.Context, t *T) {
...
func TestRequire_CancelsContextOnTimeout(t *testing.T) {
	tb := runWithFakeTB(func(tb *fakeTB) {
		Require(context.Background(), tb, func(ctx context.Context, t *T) {
			<-ctx.Done()
...
func TestRequire_ParentContextDeadlineCapsCallbackContext(t *testing.T) {
	parentCtx, cancel := context.WithTimeout(context.Background(), 50*time.Millisecond)
	defer cancel()

	tb := runWithFakeTB(func(tb *fakeTB) {
		Require(parentCtx, tb, func(ctx context.Context, t *T) {
			deadline, ok := ctx.Deadline()
			require.True(t, ok)
	...
	require.True(t, tb.Failed(), "expected the fake TB to be marked as failed")
	require.True(t, tb.hasFatal("not satisfied after"), "expected parent deadline timeout fatal message")
}

func TestRequire_ParentContextCancellationStopsPolling(t *testing.T) {
	parentCtx, cancel := context.WithCancel(context.Background())
	var attempts atomic.Int32

	tb := runWithFakeTB(func(tb *fakeTB) {
		Require(parentCtx, tb, func(_ context.Context, t *T) {
			attempts.Add(1)
			t.Error("not ready")
			cancel()
	...
	require.True(t, tb.hasFatal("context canceled before condition was satisfied"), "expected context cancellation fatal message")
}
...
func TestRequiref_IncludesMessageOnTimeout(t *testing.T) {
	tb := runWithFakeTB(func(tb *fakeTB) {
		Requiref(context.Background(), tb, func(_ context.Context, t *T) {
...
func TestRequire_PanicPropagated(t *testing.T) {
	...
	require.PanicsWithValue(t, "unexpected nil pointer", func() {
		Require(t.Context(), t, func(_ context.Context, _ *T) {
...
func TestRequire_DetectsMisuseOfRealT(t *testing.T) {
	...
	tb := runWithFakeTB(func(tb *fakeTB) {
		Require(context.Background(), tb, func(_ context.Context, _ *T) {
...
func TestRequireTrue_DetectsAssertionMisuseOfRealT(t *testing.T) {
	...
	require.True(t, tb.Failed())
	require.True(t, tb.hasFatal("do not use test assertions"), "expected misuse detection message")
}
