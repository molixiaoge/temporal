...
// failures, or until ctx is canceled or the timeout expires. Set
// TEMPORAL_TEST_AWAIT_SCALE to scale the timeout and poll interval together.
// The scaled timeout is capped at ctx's deadline and the test's deadline if
// either is set.
//
// The condition receives a derived context that is canceled when ctx is
// canceled, the timeout expires, or the test deadline is reached, plus an
...
// Example:
//
//	await.Require(ctx, t, func(ctx context.Context, t *await.T) {
//	    resp, err := client.GetStatus(ctx)
//	    require.NoError(t, err)
//	    require.Equal(t, "ready", resp.Status)
//	}, 5*time.Second, 200*time.Millisecond)
func Require(ctx context.Context, tb testing.TB, condition func(context.Context, *T), timeout, pollInterval time.Duration) {
	tb.Helper()
	run(ctx, tb, func(ctx context.Context, t *T) bool {
		condition(ctx, t)
		return true
...
// Example:
//
//	await.Requiref(ctx, t, func(ctx context.Context, t *await.T) {
//	    require.Equal(t, "ready", status.Load())
//	}, 5*time.Second, 200*time.Millisecond, "workflow %s did not reach ready state", wfID)
func Requiref(ctx context.Context, tb testing.TB, condition func(context.Context, *T), timeout, pollInterval time.Duration, msg string, args ...any) {
	tb.Helper()
	run(ctx, tb, func(ctx context.Context, t *T) bool {
		condition(ctx, t)
		return true
...
func RequireTrue(tb testing.TB, condition func() bool, timeout, pollInterval time.Duration) {
	tb.Helper()
	run(context.Background(), tb, func(context.Context, *T) bool {
		return condition()
