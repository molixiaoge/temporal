...
func (e *TestEnv) Awaitf(fn func(context.Context, *await.T), timeout, interval time.Duration, msg string, args ...any) {
	e.t.Helper()
	await.Requiref(e.Context(), e.t, fn, timeout, interval, msg, args...)
}

// AwaitTrue calls fn repeatedly until it returns true or timeout is reached.
// Use it for simple local predicates only. Do not use assertions or side
// effects in fn; return false to retry, or use Await for assertion conditions.
func (e *TestEnv) AwaitTrue(fn func() bool, timeout, interval time.Duration) {
...
func (e *TestEnv) AwaitTruef(fn func() bool, timeout, interval time.Duration, msg string, args ...any) {
	e.t.Helper()
